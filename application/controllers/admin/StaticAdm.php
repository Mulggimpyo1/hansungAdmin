...
class SmsAdm extends MY_Controller {
  ...
  public function settingProc()
  {
      ...
      $payment_use_servcie= implode(",", $this->input->post("payment_use_servcie"));
      ...
      $data = array(
        ...
        "payment_use_servcie"      => $payment_use_servcie,   
        "bank_name"                => $bank_name,             
  ...
  public function popupWrite($seq="")
  {
    ...
    */
	...
	public function surveyWriteProc()
	{
		$course_class = $this->input->post("course_class");
		$course_class = explode("|",$course_class);
		$course_code = $course_class[0];
		$class_code = $course_class[1];
		$survey_subject = $this->input->post("survey_subject");
		$survey_start_date = $this->input->post("survey_start_date");
		$survey_end_date = $this->input->post("survey_end_date");
		$reg_time = date("Y-m-d H:i:s");
		...
		$result = $this->course_model->insertSurvey($surveyData);

		$this->msg("정상적으로 등록되었습니다.");
		$this->goURL("/admin/configAdm/surveyList");
		exit;
	}
