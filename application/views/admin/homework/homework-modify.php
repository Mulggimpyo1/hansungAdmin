...
<div class="content-wrapper">
  ...
  <form id="allocationForm">
    ...
    <input type="hidden" name="homework_seq" value="{homework_seq}"/>
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="text-left align-middle">
                      <input type="text" class="form-control col-sm-6 float-left" style="margin-left:5px;margin-right:5px;" name="homework_title" id="homework_title" value="<?php echo $allocationData["homeworkData"]['homework_title']; ?>"/>
                    </td>
                  </tr>
                  <tr>
                    <th class="text-left align-middle">마감일</th>
                    <td class="text-left align-middle">
                      <input type="text" class="form-control col-sm-2 float-left date" id="final_date" name="final_date" readonly value="<?php echo date("Y-m-d",strtotime($allocationData['homeworkData']['final_date'])) ?>"/>
                    </td>
                  </tr>
                  <tr>
          ...
          <div class="card">
            ...
            <div class="card-footer clearfix">
              ...
              <span class="float-right">
                <button type="button" class="btn btn-block btn-success" onclick="allocationProc()">수정</button>
              </span>
...
var deleteUserArr = [];
  ...
  function allocationProc()
  {
    ...
    if(confirm("숙제를 배정 하시겠습니까?")){
      ...
      for(var i=0; i<newUserArr.length; i++){
        data.push({"name":"new_user_seq[]","value":newUserArr[i]});
      }
      for(var i=0; i<deleteUserArr.length; i++){
        data.push({"name":"delete_user_seq[]","value":deleteUserArr[i]});
      }
      for(var i=0; i<newContentArr.length; i++){
        data.push({"name":"new_content_seq[]","value":newContentArr[i]});
      }
      for(var i=0; i<deleteContentArr.length; i++){
        data.push({"name":"delete_content_seq[]","value":deleteContentArr[i]});
      }


      $.ajax({
        type: "POST",
        url : "/admin/academiAdm/allocationModifyProc",
        data: data,
        dataType:"json",
        traditional : true,
        success : function(data, status, xhr) {
  ...
  function addContentList($arr,$str)
  {
    for(var i=0; i<$arr.length; i++){
      contentArr.push($arr[i]);
      if($str!="first"){
        newContentArr.push($arr[i]);

        var filtered = deleteContentArr.filter((element)=>element !== $arr[i]);
        deleteContentArr = filtered;
      }

    }

    var set = new Set(contentArr);
    var set2 = new Set(newContentArr);
  ...
  function firstLoadContent()
  {
    ...
    $.ajax({
      type: "POST",
      url : "/admin/academiAdm/firstContentList/{homework_seq}",
      data: data,
      dataType:"json",
      traditional : true,
      success : function(data, status, xhr) {
        addContentList(data.data,"first");
  ...
  function deleteContent($content_seq)
  {
    ...
    var filtered2 = newContentArr.filter((element)=>element !== $content_seq);
    newContentArr = filtered2;

    deleteContentArr.push($content_seq);
  ...
  function addStudentList($arr,$str="")
  {
    ...
    for(var i=0; i<$arr.length; i++){
      ...
      if($str!="first"){
        ...
        var filtered = deleteUserArr.filter((element)=>element !== $arr[i]);
        deleteUserArr = filtered;
      }

    }

    var set = new Set(studentArr);
    var set2 = new Set(newUserArr);
  ...
  function firstLoadStudent()
  {
    ...
    $.ajax({
      type: "POST",
      url : "/admin/academiAdm/firstStudentList/{homework_seq}",
      data: data,
      dataType:"json",
      traditional : true,
      success : function(data, status, xhr) {
        addStudentList(data.data,"first");
  ...
  function deleteStudent($user_seq)
  {
    ...
    var filtered2 = newUserArr.filter((element)=>element !== $user_seq);
    newUserArr = filtered2;

    deleteUserArr.push($user_seq);
