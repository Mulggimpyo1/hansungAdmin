		...
		<div class="ranking_wrap">
          ...
          <li><a href="/rank/achieve">성취도</a></li>
              ...
              <li><a href="/rank/ranking">개인 랭킹</a></li>
							<li><a href="/rank/rankClass" class="on">학급 랭킹</a></li>
		...
		if($data.rankArr.length>0){
			for(var i = 0; i<$data.rankArr.length; i++){
				...
				if($data.rankArr[i].school_name==null){
					...
					html += '<span class="user">'+$data.rankArr[i].school_name+' '+$data.rankArr[i].school_year+'-'+$data.rankArr[i].school_class+'</span>';
				}
			...
			if($data.myRank > 10){
				var rate = ($data.myRank / $data.total_point_class) * 100;
				html += '<li class="my_rank">';
				html += '<span class="rank_num">'+$data.myRank+'</span>';
				html += '<span class="user">'+$data.userData.school_name+' '+$data.userData.school_year+'-'+$data.userData.school_class+'<span class="rate">상위'+rate+'%</span></span>';
